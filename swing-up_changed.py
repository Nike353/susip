...
k = 0.2	# control gain coefficient
u_max = 5
...
x0 = np.array([xs[temp],ths[temp],vs[temp],Ys[temp]])
desired_poles = np.array([-1,-3,-4,-10])
K = ctl.place(A, B, desired_poles)

# Create the state-space system with feedback
sys_cl = ctl.ss(A - B @ K, B, C, D)

# Simulate the system's response to a step input
t_cl = np.linspace(0, 5, 500)

t_cl, y, sol = cnt.lsim(sys_cl, T=t_cl,X0 = x0)
#print(x)
# K = (np.matrix(np.real(np.linalg.eig(A - B @ (np.matrix(ctl.place(A, B, desired_poles))))[0])))
# print(K)
# def control_law(x,t):
# 	u = -K @ (np.transpose(np.matrix(x)) - np.transpose(np.matrix(x_eq)))
# 	return np.array(u)[0]
...
# 	dxdt[0] = x[2]
# 	dxdt[1] = x[3]
# 	dxdt[2] = (u+m*L*(x[3]**2)*np.sin(x[1])/2 -3*m*g*np.sin(2*x[1])/8)/(M+m-3*m*(np.cos(x[1])**2)/4) 
# 	dxdt[3] = (3/2*L)*(g*np.sin(x[1])-dxdt[2]*np.cos(x[1]))
# 	return dxdt


# sol = integrate.odeint(dynamics, x0, t)

fig = pp.figure()
ax = fig.add_subplot(111, autoscale_on=True, xlim=(-2, 2), ylim=(-1.2, 2))
ax.set_aspect('equal')
ax.grid()

patch = ax.add_patch(Rectangle((0, 0), 0, 0, linewidth=2, edgecolor='k', facecolor='g'))

line, = ax.plot([], [], 's-', lw=1,color = 'g')
...
pp.figure()
pp.subplot(211)
t = np.arange(0.0, len(xs_final)/20, dt)
#t = np.arange(0.0, Tmax, dt)
Es = np.vectorize(energy)(ths_final, Ys_final)
Us = k * Es * Ys_final * cos(ths_final)

# pp.plot(t, Us, label='U')
#pp.plot(t, vs_final, label='v')
#pp.plot(t, xs_final, label='x')
pp.plot(t, ths_final, label=r"$\Theta$")
#pp.plot(t, Ys_final, label="th'")
pp.grid(True)
pp.legend()
pp.subplot(212)
